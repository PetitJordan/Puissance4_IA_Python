...
def initial_plateau(plateau):
    ...
    for c in range (NB_COLONNE):
        for r in range(NB_LIGNE):
            ...
            pygame.draw.circle(ecran,NOIR,(int(c*LARGEUR_CASE+LARGEUR_CASE/2), int(r*LARGEUR_CASE+LARGEUR_CASE/2+LARGEUR_CASE)),RAYON)
    for c in range (NB_COLONNE):
        for r in range(NB_LIGNE):            
            if plateau[r][c] == ORDINATEUR:
                pygame.draw.circle(ecran,COULEUR_ORDI,(int(c*LARGEUR_CASE+LARGEUR_CASE/2),hauteur- int(r*LARGEUR_CASE+LARGEUR_CASE/2)),RAYON)
            if plateau[r][c] == HUMAIN:
                pygame.draw.circle(ecran,COULEUR_HUMAIN,(int(c*LARGEUR_CASE+LARGEUR_CASE/2),hauteur- int(r*LARGEUR_CASE+LARGEUR_CASE/2)),RAYON)
            if plateau[r][c] == GAGNANT_ORDI:
                pygame.draw.circle(ecran,COULEUR_ORDI,(int(c*LARGEUR_CASE+LARGEUR_CASE/2),hauteur- int(r*LARGEUR_CASE+LARGEUR_CASE/2)),RAYON)
                pygame.draw.circle(ecran,COULEUR_GAGNANT,(int(c*LARGEUR_CASE+LARGEUR_CASE/2),hauteur- int(r*LARGEUR_CASE+LARGEUR_CASE/2)),RAYON-15)
...
def intro_jeu(ecran):
    ...
    while intro:
        ...
        texteSurface = font.render("Contre quel niveau souhaitez-vous jouer ?", True, BLANC)
        ...
        button(ecran,"Facile",200,(hauteur_ecran/2),100,50,MARRON,MARRON_CLAIR, jouer_niveau1)
        button(ecran,"Moyen",330,(hauteur_ecran/2),100,50,ROUGE,ROUGE_CLAIR, jouer_niveau2)
        button(ecran,"Elevé",460,(hauteur_ecran/2),100,50,BLEU,BLEU_CLAIR, jouer_niveau3)
        button(ecran,"Quitter",600,(hauteur_ecran/2),100,50,VIOLET,VIOLET_CLAIR, jouer_fin)
        pygame.display.update()
